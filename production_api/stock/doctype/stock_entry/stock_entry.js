			...
			if (frm.doc.purpose=='Send to Warehouse' && frm.doc.per_transferred < 100) {
				frm.add_custom_button(__('End Transit'), function() {
					frappe.model.open_mapped_doc({
						method: "production_api.stock.doctype.stock_entry.stock_entry.make_stock_in_entry",
						frm: frm
					})
				});
			}
...
production_api.stock.StockEntry = class StockEntry extends frappe.ui.form.Controller {
	refresh() {
...
extend_cscript(cur_frm.cscript, new production_api.stock.StockEntry({frm: cur_frm}));
