...
frappe.ui.form.on("Repost Item Valuation", {
	...
	based_on: function (frm) {
		...
		if (frm.doc.based_on == "Transaction") {
			fields_to_reset = ["item_code", "warehouse", "lot"];
	...
	execute_reposting(frm) {
		frm.add_custom_button(__("Start Reposting"), () => {
			frappe.call({
				method: "production_api.mrp_stock.doctype.repost_item_valuation.repost_item_valuation.execute_repost_item_valuation",
				callback: function () {
