...
def format_report_data(filters: Filters, item_details: Dict, to_date: str) -> List[Dict]:
	...
	for item, item_dict in item_details.items():
		...
		# row = [details.name, details.item_name, details.item_group, details.brand]
		row = {
			"item": details.name,
		...
		if filters.get("show_warehouse_wise_stock"):
			row.update({"warehouse": details.warehouse})
...
def get_chart_data(data: List, filters: Filters) -> Dict:
	...
	data.sort(key=lambda row: row["average_age"], reverse=True)
	...
	for row in data:
		...
		datapoints.append(row["average_age"])
...
class FIFOSlots:
	...
	def __compute_incoming_stock(
		self, row: Dict, fifo_queue: List, transfer_key: Tuple
	):
