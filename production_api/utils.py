...
def get_finishing_plan_list(finishing_items):
	...
	for key in finishing_items:
		...
		finshing_items_list.append({
			...
			"transferred_qty": finishing_items[key]['transferred_qty'],
...
def get_work_in_progress_report(category, status, lot_list_val, item_list, process_list, from_date=None, to_date=None):
	...
	lot_dict = {
		"columns": {
			...
			"sew_columns": {
				...
				"Sewing Received": "sewing_received",
	...
	for lot in lot_list:
		...
		lot_dict['lot_data'][lot]['sewing_details']['in_sew'] = lot_dict['lot_data'][lot]['sewing_details']['sewing_received'] - lot_dict['lot_data'][lot]['sewing_details']['sewing_sent']
		...
		if finishing_plan_list:
			...
			lot_dict['total_data']['sewing_details']['in_sew'] = (
				lot_dict['total_data']['sewing_details']['sewing_received']
				- lot_dict['total_data']['sewing_details']['sewing_sent']
			)
			lot_dict['lot_data'][lot]['sewing_details']['in_sew'] = (
				lot_dict['lot_data'][lot]['sewing_details']['sewing_received']
		...
		if not finishing_plan_list:
			if packing_wos:
				...
				lot_dict['total_data']['finishing_details']['dispatch'] += dispatch_qty
				lot_dict['lot_data'][lot]['finishing_details']['dispatch'] += dispatch_qty

		cut_to_dispatch_diff = (
			lot_dict['total_data']['finishing_details']['dispatch'] +
			lot_dict['total_data']['finishing_details']['transferred'] - 
			lot_dict['total_data']['cut_details']['cut_qty']
		)
		cut_to_dispatch_lot_diff = (
			lot_dict['lot_data'][lot]['finishing_details']['transferred'] +
			lot_dict['lot_data'][lot]['finishing_details']['dispatch'] - 
			lot_dict['lot_data'][lot]['cut_details']['cut_qty']
		)
		...
		lot_dict['total_data']['finishing_details']['cut_to_dispatch_diff'] = cut_to_dispatch_diff
		...
		lot_dict['lot_data'][lot]['finishing_details']['cut_to_dispatch_diff'] = cut_to_dispatch_lot_diff
		...
		for process in process_list:
			...
			for wo in wo_list:
				...
				if detail:
					...
					lot_dict['lot_data'][lot][details_key][diff_val] += (detail[0]['received'] - detail[0]['sent'])
					lot_dict['total_data'][details_key][sent_val] += detail[0]['sent']
					lot_dict['total_data'][details_key][rec_val] += detail[0]['received']
					lot_dict['total_data'][details_key][diff_val] += (detail[0]['received'] - detail[0]['sent'])
	...
	lot_dict["summary"] = {
		...
		"total_order_to_cut_diff": lot_dict["total_data"]["cut_details"]["order_to_cut_diff"],
		"total_cut_to_sew_diff": lot_dict["total_data"]["sewing_details"]["cut_to_sew_diff"],
		"total_in_sew": lot_dict["total_data"]["sewing_details"]["in_sew"],
		"total_cut_to_dispatch_diff": lot_dict["total_data"]["finishing_details"]["cut_to_dispatch_diff"],
		"total_in_packing": lot_dict["total_data"]["finishing_details"]["in_packing"],
		"total_ready_for_packing": lot_dict["total_data"]["sewing_details"]["ready_for_packing"],
		"dynamic_col": [],
	}
	for clo_key, detail_key in [("against_cut_columns","against_cut_details"), ("against_sew_columns","against_sew_details")]:
		for label,val in lot_dict['columns'].get(clo_key,{}).items():
			if not label.endswith(" Diff"):
				continue
			if label.endswith(" to Cut Diff") or label.endswith(" to Sew Diff"):
				continue
...
def get_size_wise_stock_report(open_status, lot_list, item_list, category, process_list):
	...
	for lot in lot_list:
		...
		for row in lot_doc.lot_order_details:
			...
			lot_dict['lot_data'][lot]['finishing_details']['transferred'].setdefault(size, 0)
		...
		for finishing_plan in finishing_plan_list:
			...
			for row in fp_doc.finishing_plan_details: 
				attr_details = get_variant_attr_details(row.item_variant)
				size = attr_details[primary]
				lot_dict['lot_data'][lot]['finishing_details']['transferred'][size] += row.transferred_qty
		...
		for size in sizes:		
			...
			transferred = lot_dict['lot_data'][lot]['finishing_details']['transferred'][size]
			#diff detail
			...
			lot_dict['lot_data'][lot]['finishing_details']['work_in_progress'][size] += (work_in_progress - dispatch_qty - transferred)
			#total details
			...
			lot_dict['lot_data'][lot]['total_details']["work_in_progress_total"] += (work_in_progress - dispatch_qty - transferred)
	...
	for lot, lot_data in lot_dict['lot_data'].items():
		...
		def merge_size_dict(target, source):
			for size, qty in source.items():
				target[size] = target.get(size, 0) + qty

		def merge_nested_size_dict(target, source):
			for key, size_dict in source.items():
				inner_target = target.setdefault(key, {})
				for size, qty in size_dict.items():
					inner_target[size] = inner_target.get(size, 0) + qty

		for key in ["order_qty", "cut_qty", "order_to_cut_diff"]:
			merge_size_dict(style_entry["cut_details"][key], lot_data["cut_details"].get(key, {}))

		merge_nested_size_dict(style_entry["against_cut_details"], lot_data.get("against_cut_details", {}))

		for key in ["sewing_sent", "cut_to_sew_diff", "finishing_inward", "in_sew"]:
			merge_size_dict(style_entry["sewing_details"][key], lot_data["sewing_details"].get(key, {}))

		merge_nested_size_dict(style_entry["against_sew_details"], lot_data.get("against_sew_details", {}))

		for key in ["dispatch", "in_packing", "transferred", "cut_to_dispatch_diff", "work_in_progress"]:
...
def get_colour_wise_diff_report(lot, process_list):
	...
	for row in lot_doc.lot_order_details:
		...
		lot_dict['colours'].append(colour)	
	...
	for process in process_list:
		...
		if not wo_list:
			for colour in lot_dict['values']:
				for size in lot_dict['sizes']:
					lot_dict['values'][colour]['supplier_details'].setdefault(sent_val, [])
					lot_dict['values'][colour]['rows'][row_key].setdefault(sent_key, sent_val)
					lot_dict['values'][colour]['rows'][row_key].setdefault(rec_key, rec_val)
					lot_dict['values'][colour]['rows'][row_key].setdefault(diff_key, diff_val)
		...
		for wo in wo_list:
			...
			for row in wo_doc.work_order_calculated_items:
				...
				size = attr_details[primary]
				colour = get_variant_set_colour(attr_details, row.set_combination, is_set_item, pack_attr, set_attr)				
				if colour not in process_colours:
					process_colours.append(colour)
				...
				if process_dict[process_name] == cut:
					if not colour_wise_last_process[colour]['last_cut_process']:
						...
						lot_dict['values'][colour]['rows'][row_key].setdefault(against_key, against_val)
		...
		if wo_list:
			for clr in lot_dict['colours']:
				if clr in process_colours:
					continue
				for size in lot_dict['sizes']:
					lot_dict['values'][clr]['supplier_details'].setdefault(sent_val, [])
					lot_dict['values'][clr]['rows'][row_key].setdefault(sent_key, sent_val)
					lot_dict['values'][clr]['rows'][row_key].setdefault(rec_key, rec_val)
					lot_dict['values'][clr]['rows'][row_key].setdefault(diff_key, diff_val)
					lot_dict['values'][clr][details_key].setdefault(sent_val, {})
					lot_dict['values'][clr][details_key].setdefault(rec_val, {})
					lot_dict['values'][clr][details_key].setdefault(diff_val, {})
					lot_dict['values'][clr][details_key][sent_val].setdefault(size, 0)
					lot_dict['values'][clr][details_key][rec_val].setdefault(size, 0)
					lot_dict['values'][clr][details_key][diff_val].setdefault(size, 0)
					
					lot_dict['values'][clr]['total_details'].setdefault(total_sent_key, 0)
					lot_dict['values'][clr]['total_details'].setdefault(total_rec_key, 0)
					lot_dict['values'][clr]['total_details'].setdefault(total_diff_key, 0)
	return lot_dict
...
@frappe.whitelist()
def get_lotwise_cloth_usage_report(item_list, lot_list):
	conditions = ""
	con = {}
...
def dc_dpr_report(date=None, lot=None, item=None, dc_name=None):
	...
	finishing_plan_ironing=frappe.db.get_single_value("MRP Settings", "finishing_plan_ironing_process")
	dc_process=frappe.db.sql("""
						  SELECT parent FROM `tabProcess Details` WHERE process_name = %(finishing_plan_ironing)s
						  """, {"finishing_plan_ironing": finishing_plan_ironing}, as_dict=True)
	dc_p=[finishing_plan_ironing]
	for dc in dc_process:
		dc_p.append(dc.parent)	

	if dc_p:
		conditions += " AND t1.process_name IN %(dc_p)s"
		con["dc_p"] = tuple(dc_p)
