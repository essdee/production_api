...
def get_columns():
	return [
		...
		{"fieldname": "received_location", "fieldtype": "Link", "options": "Supplier", "label": "Received Location", "width": 140},
		{"fieldname": "received_location_name", "fieldtype": "Data", "label": "Received Location Name", "width": 200},
...
def get_data(filters):
	...
	conditions = [
		"se.docstatus = 1",
		"sed.docstatus = 1",
		"se.purpose IN ('Material Receipt', 'Send to Warehouse')",
		"IFNULL(se.against, '') = ''",
		"IFNULL(se.to_warehouse, '') != ''",
	]
	supplier_expression = "se.transfer_supplier"
	from_location_expression = "COALESCE(NULLIF(se.from_warehouse, ''), se.transfer_supplier)"
	...
	if filters.get("received_location"):
		conditions.append("se.to_warehouse = %(received_location)s")
		params["received_location"] = filters.received_location
	...
	query = """
		SELECT
			...
			{from_location_expression} AS from_location,
			from_supplier.supplier_name AS from_location_name,
			se.to_warehouse AS received_location,
			received_supplier.supplier_name AS received_location_name,
		...
		LEFT JOIN `tabSupplier` from_supplier ON from_supplier.name = {from_location_expression}
		LEFT JOIN `tabSupplier` received_supplier ON received_supplier.name = se.to_warehouse
	...
	""".format(
		...
		from_location_expression=from_location_expression,
	)
