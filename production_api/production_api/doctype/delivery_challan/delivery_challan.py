...
class DeliveryChallan(Document):
    def before_cancel(self):
        ...
        cp_list = frappe.get_all("Cutting Plan", filters={
                                 "work_order": self.work_order}, pluck="name")
    ...
    def on_cancel(self):
        ...
        dept_attr, piece_stage, pack_attr = frappe.get_value("Item Production Detail", self.production_detail, [
        ...
        for row in self.items:
            if res.get(row.item_variant):
                add_sl_entries.append(self.get_sle_data(
                ...
                if self.includes_packing and check_dependent_stage_variant(row.item_variant, dept_attr, piece_stage):
                    updated_variant = get_lpiece_variant(
                        pack_attr, dept_attr, row.item_variant)
                    reduce_sl_entries.append(self.get_sle_data(
                        row, supplier, 1, {}, received_type, new_variant=updated_variant))
                else:
                    reduce_sl_entries.append(self.get_sle_data(
                        row, supplier, 1, {}, received_type))

        logger.debug(f"{self.name} SLE data construction {datetime.now()}")
        make_sl_entries(add_sl_entries)
        logger.debug(
            f"{self.name} Stock Added to From Location {datetime.now()}")
        make_sl_entries(reduce_sl_entries)
        logger.debug(
            f"{self.name} Stock reduced From Supplier {datetime.now()}")
    ...
    def before_submit(self):
        ...
        if self.allow_non_bundle:
            ...
            if not role:
                frappe.throw(
                    "Set the role for Non Bundle DC submit in MRP Settings", title='GRN')
            user_roles = frappe.get_roles(frappe.session.user)
            if role not in user_roles:
                frappe.throw(
                    f"Only {role} can submit this document.", title='DC')
        ...
        for row in self.items:
            if res.get(row.item_variant) and row.delivered_quantity > 0:
                ...
                quantity, rate = get_stock_balance(
                    row.item_variant, self.from_location, received_type, with_valuation_rate=True, lot=self.lot)
        ...
        logger.debug(
            f"{self.name} Stock check and SLE data construction {datetime.now()}")
        wo_doc = frappe.get_cached_doc("Work Order", self.work_order)
        for deliverable in wo_doc.deliverables:
            for item in self.items:
                if item.ref_docname == deliverable.name:
                    deliverable.pending_quantity = '{0:.3f}'.format(
                        deliverable.pending_quantity - item.get('delivered_quantity'))
        ...
        cp_list = frappe.get_all("Cutting Plan", filters={
                                 "work_order": work_order}, pluck="name")
        if cp_list and (self.from_address == self.supplier_address or not self.is_internal_unit):
            cp_doc = frappe.get_doc("Cutting Plan", cp_list[0])
            for dc_item in self.items:
                check = False
                ...
                if not check:
                    parent_item = frappe.get_value(
                        "Item Variant", dc_item.item_variant, "item")
        ...
        make_sl_entries(reduce_sl_entries)
        logger.debug(
            f"{self.name} Stock reduced from From Location {datetime.now()}")
        make_sl_entries(add_sl_entries)
    ...
    def onload(self):
        if self.get('items'):
            if self.is_rework:
                deliverable_item_details = fetch_rework_dc_item_details(
                    self.get('items'), self.production_detail, self.lot)
            else:
    ...
    def before_validate(self):
        ...
        docstatus = frappe.get_value(
            "Work Order", self.work_order, "docstatus")
        if docstatus != 1:
        ...
        if (self.get('deliverable_item_details')):
            ...
            if self.is_rework:
                deliverables, stock_value = save_rework_deliverables(
                    self.deliverable_item_details, self.from_location, self.cut_panel_movement)
            else:
                rec_type = frappe.db.get_single_value(
                    "Stock Settings", "default_received_type")
                deliverables, stock_value = save_deliverables(
                    self.deliverable_item_details, self.from_location, self.cut_panel_movement, received_type=rec_type)
    ...
    def validate(self):
        ...
        for row in self.items:
            item_details = get_uom_details(
                row.item_variant, row.uom, row.delivered_quantity)
...
def save_rework_deliverables(item_details, from_location, cut_panel_movement):
    ...
    for received_type in item_details:
        items, stock_value = save_deliverables(
            item_details[received_type], from_location, cut_panel_movement, received_type=received_type)
            ...
            if (item.get('primary_attribute')):
                ...
                for attr, values in item['values'].items():
                    if values.get('qty') or values.get('delivered_quantity'):
                        ...
                        item1['delivered_quantity'] = values.get(
                            'delivered_quantity')
                        item1['pending_quantity'] = item1['qty'] - \
                        ...
                        stock = get_stock_value(
                            variant_name, item.get('lot'), from_location)
                ...
                if item['values']['default'].get('qty') or item['values']['default'].get('delivered_quantity'):
                    ...
                    item1['secondary_qty'] = item['values']['default'].get(
                        'secondary_qty')
                    item1['secondary_uom'] = item['values']['default'].get(
                        'secondary_uom')
                    item1['lot'] = item.get('lot')
                    item1['delivered_quantity'] = item['values']['default'].get(
                        'delivered_quantity')
                    item1['pending_quantity'] = item1['qty'] - \
                        item1['delivered_quantity']
                    item1['uom'] = item.get('default_uom')
                    ...
                    item1['set_combination'] = item['values']['default'].get(
                        'set_combination', {})
                    item1['comments'] = item['values']['default'].get(
                        'comments', None)
                    stock = get_stock_value(
                        variant_name, item.get('lot'), from_location)
                    item1['stock_value'] = stock
...
def fetch_rework_dc_item_details(items, ipd, lot):
    ...
    for received_type, grp_variants in groupby(items, lambda i: i['item_type']):
        item_details = fetch_item_details(
            grp_variants, ipd, lot, is_rework=True)
    ...
    for key, variants in groupby(items, lambda i: i['row_index']):
        ...
        if item['primary_attribute']:
            for attr in current_item_attribute_details['primary_attribute_values']:
                item['values'][attr] = {
                    'qty': 0, 'rate': 0, 'delivered_quantity': 0}
                ...
                for attr in current_variant.attributes:
                    if attr.attribute == item.get('primary_attribute'):
                        ...
                        if is_new:
                            ...
                            item['values'][attr.attribute_value]['delivered_quantity'] = variant.get(
            ...
            if is_new:
                ...
                item['values']['default']['delivered_quantity'] = variants[0].get(
                    "delivered_quantity", 0)
...
def get_deliverables(work_order):
    ...
    if doc.is_rework:
        items = fetch_rework_item_details(
            doc.deliverables, doc.production_detail, is_dc=True)
    else:
        items = fetch_item_details(
            doc.deliverables, doc.production_detail, doc.lot, is_new=True)
    return {
        "items": items,
...
def get_dc_structure(doc_name):
    ...
    item_details = fetch_item_details(
        doc.items, doc.production_detail, doc.lot)
    exp_date = frappe.get_value(
        "Work Order", doc.work_order, "expected_delivery_date")
...
def get_calculated_items(doc_name, work_order):
    ...
    items = fetch_order_item_details(
        doc.work_order_calculated_items, doc.production_detail, includes_packing=doc.includes_packing)
    return items
...
def get_calculated_deliverables(items, wo_name, doc_name, deliverable):
    ...
    logger = get_module_logger("delivery_challan")
    logger.debug(
        f"{doc_name} Deliverable Calculation Started {datetime.now()}")
...
# work order date update
    ...
    if process_name == ipd_doc.cutting_process:
        ...
        incomplete_items, completed_items, total_delivered, qty_list = calculate_cutting_piece(
            dc_doc, panel_list)

    elif dc_doc.includes_packing:
        final_calculation, total_delivered = calculate_piece_stage(
            dc_doc, doc_status, total_delivered, final_calculation, wo_doc.item)
    else:
            ...
            if finishing_doc:
                ...
                for row in wo_doc.work_order_calculated_items:
                    key = (row.item_variant, tuple(
                        sorted(update_if_string_instance(row.set_combination).items())))
...
def calculate_cutting_piece(dc_doc, panel_list):
    ...
    production_detail, incomplete_items_json, completed_items_json = frappe.get_cached_value(
        "Work Order", dc_doc.work_order, ['production_detail', "wo_delivered_incompleted_json", "wo_delivered_completed_json"])
    ...
    for item in dc_doc.items:
        ...
        for i in incomplete_items['items']:
            ...
            if con1 and con2 and item.delivered_quantity > 0:
                ...
                y = i.copy()
                if not x:
                    i['values'][attrs[ipd_doc.primary_item_attribute]
                                ][attrs[ipd_doc.stiching_attribute]] = item.delivered_quantity
                else:
            ...
            if (item.get('primary_attribute')):
                for attr, values in item['values'].items():
                    if values.get('return_quantity'):
                        ...
                        item1['item_variant'] = variant_name
                        item1['lot'] = item.get('lot')
                        item1['return_quantity'] = values.get(
                            'return_quantity')
                        item1['uom'] = item.get('default_uom')
                        item1['table_index'] = table_index
                        item1['row_index'] = row_index
                        ...
                        item1['set_combination'] = values.get(
                            'set_combination', {})
                        item1['received_type'] = item.get(
                            "item_type", default_received_type)
                        items.append(item1)
            else:
                if item['values']['default'].get('return_quantity'):
                    ...
                    item1['item_variant'] = variant_name
                    item1['lot'] = item.get('lot')
                    item1['return_quantity'] = item['values']['default'].get(
                        'return_quantity')
                    item1['uom'] = item.get('default_uom')
                    item1['table_index'] = table_index
                    item1['row_index'] = row_index
...
def update_finishing_item_doc(doc_name, finishing_doc_name, update_dc: bool):
    ...
    if self.is_internal_unit and self.supplier_address != self.from_address:
        incomplete_transfer_dc_list = update_if_string_instance(
            finishing_doc.incomplete_transfer_dc_list)
        if self.docstatus == 2:
