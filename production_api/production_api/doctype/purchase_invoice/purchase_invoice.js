...
frappe.ui.form.on("Purchase Invoice", {
  ...
  refresh: function (frm) {
    if (frm.doc.docstatus == 1 && frm.doc.erp_inv_name) {
      frm.add_custom_button(__("Show Bill"), function () {
        frappe.call({
          method:
            "production_api.production_api.doctype.purchase_invoice.purchase_invoice.get_erp_inv_link",
          args: {
            name: frm.doc.name,
          },
          callback: function (r) {
    ...
    if (frm.doc.docstatus == 1 && frm.doc.erp_inv_docstatus == 0) {
      frm.add_custom_button(__("Submit Bill"), function () {
        frappe.call({
          method:
            "production_api.production_api.doctype.purchase_invoice.purchase_invoice.submit_erp_invoice",
          args: {
            name: frm.doc.name,
          },
          freeze: true,
    ...
    if (frm.doc.__onload && frm.doc.__onload.item_details) {
      frm.pi_wo_items = new frappe.production.ui.InvoiceWoItems(
        frm.fields_dict["work_order_details_html"].wrapper,
      );
  ...
  fetch_grn: function (frm) {
    ...
    if (
      ...
      frm.doc.purchase_invoice_wo_approval_details.length > 0
    ) {
      let d = new frappe.ui.Dialog({
        title:
          "Fetching the GRN will revert all approvals. Do you want to continue?",
  ...
  fetch_expense_head(frm) {
    if (frm.doc.items.length == 0) {
      ...
      frappe.call({
        method:
          "production_api.production_api.doctype.purchase_invoice.purchase_invoice.fetch_items_expense_head",
        args: {
          items: frm.doc.items,
        },
        callback: function (r) {
...
function fetch_grn(frm) {
  ...
  frappe.call({
    method:
      "production_api.production_api.doctype.purchase_invoice.purchase_invoice.fetch_grn_details",
    args: {
      grns: grns,
