...
class WorkOrder(Document):
    ...
    def set_status(self):
        ...
        total_delivery_pending = 0

        for d in self.deliverables:
            total_delivery_qty += flt(d.qty)
            total_delivery_pending += flt(d.pending_quantity)

        if self.deliverables and total_delivery_qty > 0:
            if total_delivery_pending <= 0:
                status = "Fully Delivered"
            elif total_delivery_pending < total_delivery_qty:
                status = "Partially Delivered"

        total_received_qty = 0
        total_received_pending = 0

        for r in self.receivables:
            total_received_qty += flt(r.qty)
            total_received_pending += flt(r.pending_quantity)

        if self.receivables and total_received_qty > 0:
            received_qty = total_received_qty - total_received_pending
            if received_qty > 0:
                if total_received_pending <= 0:
        ...
        if total_received_qty > 0 and total_billed_qty > 0:
            if total_billed_qty >= total_received_qty:
                status = "Fully Billed"
            else:
    ...
    def on_submit(self):
        ...
        if self.includes_packing:
            is_transferred, transferred_lot = frappe.get_value(
                "Lot", self.lot, ["is_transferred", "transferred_lot"])
            if is_transferred:
                reduce_items = []
                update_items = []
                ...
                if not fp_list:
                    frappe.throw(
                        f"There is No Finishing Plan for Parent Lot {transferred_lot}")
                fp_doc = frappe.get_doc("Finishing Plan", fp_list[0])
                fp_item = fp_doc.item
                fp_uom = frappe.get_value(
                    "Item", fp_doc.item, "default_unit_of_measure")
                self_uom = frappe.get_value(
                    "Item", self.item, "default_unit_of_measure")
                ...
                for row in self.deliverables:
                    ...
                    if item == self.item and row.qty > 0:
                        ...
                        set_comb = update_if_string_instance(
                            row.set_combination)
                ...
                self.db_set("reduce_stock_entry", create_bulk_stock_entry(
                    {"from_warehouse": self.supplier}, reduce_items, "Material Issue", submit=True))
                self.db_set("update_stock_entry", create_bulk_stock_entry(
                    {"to_warehouse": self.supplier}, update_items, "Material Receipt", submit=True))
    ...
    def auto_create_box_sticker_print(self):
        ...
        # A manual special-rate sticker must not suppress the PPO-priced stickers
        # generated by the Work Order.
        existing_bsp = frappe.db.exists("Box Sticker Print", {
            "lot": self.lot,
            "docstatus": 1,
            "against": "Work Order",
            "against_id": ["!=", ""],
                ...
                if is_transferred:
                    ...
                    for row in self.deliverables:
                        ...
                        if item == self.item:
                            ...
                            set_comb = update_if_string_instance(
                                row.set_combination)
                            key = (new_variant, tuple(
                                sorted(set_comb.items())))
                            fp_dict[key]['transferred_qty'] -= row.qty

                    se_doc = frappe.get_doc(
                        "Stock Entry", self.update_stock_entry)
                    se_doc.cancel()
                    se_doc = frappe.get_doc(
                        "Stock Entry", self.reduce_stock_entry)
                    se_doc.cancel()
                    fp_list = get_finishing_plan_list(fp_dict)
                    fp_doc.set("finishing_plan_details", fp_list)
                    fp_doc.save(ignore_permissions=True)
                ...
                for grn in grn_details:
                    ...
                    if self.docstatus == 1:
                        ...
                        if qty >= rework_quantity:
                            ...
                            qty = qty - update_qty
                    frappe.db.sql(
                        f"UPDATE `tabGoods Received Note Item` SET rework_quantity = {update_qty} WHERE name = '{grn}'")

    def onload(self):
        if not self.is_new():
            cp = frappe.get_value("Item Production Detail",
                                  self.production_detail, "cutting_process")
        ...
        if self.is_rework:
            deliverable_item_details = fetch_rework_item_details(
                self.get('deliverables'), self.production_detail)
            self.set_onload('deliverable_item_details',
                            deliverable_item_details)
        else:
            deliverable_item_details = fetch_item_details(
                self.get('deliverables'), self.production_detail, is_calc=True)
            self.set_onload('deliverable_item_details',
                            deliverable_item_details)

        receivable_item_details = fetch_item_details(
            self.get('receivables'), self.production_detail, include_id=True)
    ...
    def before_cancel(self):
        ...
        if dc_list:
            frappe.throw(_("Cannot cancel because Delivery Challan {0} is submitted against this Work Order").format(
                ", ".join(dc_list)))
    ...
    def before_validate(self):
        ...
        if not self.is_new():
            process = frappe.db.get_value(
                "Work Order", self.name, "process_name")
        ...
        if calculate:
            ...
            if (self.get('receivable_item_details')):
                items = save_item_details(
                    self.receivable_item_details, supplier=self.supplier, process_name=self.process_name)
    ...
    def calc_receivable_rate(self, get_name=False):
        ...
        if is_group:
            ...
            main_prs = [ipd_doc.packing_process,
                        ipd_doc.stiching_process, ipd_doc.cutting_process]
        ...
        ipd_doc = frappe.get_cached_doc(
            "Item Production Detail", self.production_detail)
        stich_details = {}
        ...
        for row in self.receivables:
            rate, attributes = get_rate_and_quantity(
                row.item_variant, row.qty, self, process_doc)
            attr_qty = get_attributes_qty(
                ipd_doc, self.process_name, process_doc.depends_on_attribute)
            if check:
                rate = rate / 2
            rate = rate/attr_qty
            if stich_details:
                if attributes.get(ipd_doc.stiching_attribute):
                    rate = rate / \
                        stich_details[attributes.get(
                            ipd_doc.stiching_attribute)]
        ...
        for item in group['items']:
            ...
            if (item.get('primary_attribute')):
                for attr, values in item['values'].items():
                    ...
                    item1 = get_data(item, table_index, row_index,
                                     process_name, quantity, cost)
                    ...
                    if not supplier:
                        item1['is_calculated'] = values.get(
                            'is_calculated') or 0
                    items.append(item1)
            else:
                if item['values'].get('default') and item['values']['default'].get('qty'):
                    ...
                    item1['secondary_qty'] = item['values']['default'].get(
                        'secondary_qty')
                    if not supplier:
                        item1['is_calculated'] = item['values']['default'].get(
                            'is_calculated') or 0
...
def get_rate_and_quantity(variant_name, quantity, doc, process_doc):
    ...
    if process_doc.depends_on_attribute:
        ...
        attribute_value = next(
            (attr.attribute_value for attr in item_doc.attributes if attr.attribute == attribute), None)
                            ...
                            if attr.attribute == item.get('primary_attribute'):
                                ...
                                if is_dc:
                                    item['values'][attr.attribute_value]['qty'] = round(
                                        variant['pending_quantity']) if variant['pending_quantity'] else 0
                                break
    ...
    for key, variants in groupby(items, lambda i: i['row_index']):
        ...
        if item['primary_attribute']:
            ...
            for variant in variants:
                ...
                for attr in current_variant.attributes:
                    if attr.attribute == item.get('primary_attribute'):
                        ...
                        if is_grn:
                            item['values'][attr.attribute_value]['qty'] = round(
                                variant.pending_quantity, 0)
                            item['values'][attr.attribute_value]['rate'] = round(
                                variant.cost, 2) if variant.cost else variant.cost
            ...
            if is_grn:
                ...
                item['values']['default']['rate'] = round(
                    variants[0].cost, 2) if variants[0].cost else variants[0].cost
...
def get_work_order_items(work_order, is_grn=False):
    ...
    data = fetch_item_details(
        wo.receivables, wo.production_detail, include_id=True, is_grn=is_grn)
    return data
...
def get_lot_items(lot, doc_name, process, includes_packing=False):
    ...
    items = fetch_order_item_details(
        doc.lot_order_details, doc.production_detail, process=process, includes_packing=includes_packing)
...
def get_deliverable_receivable(items, doc_name, deliverable=False, receivable=False, is_alternate=False):
    logger = get_module_logger("work_order")
    logger.debug(
        f"{doc_name} Deliverable and Receivable Calculation {datetime.now()}")
    ...
    wo_colour_sets = get_report_data(
        items, dept_attribute, ipd_doc.packing_attribute)
    ...
    item_list, row_index, table_index = get_item_structure(
        items, item_name, wo_doc.process_name, uom)
    deliverables = []
    receivables = []
    total_qty = 0
    ...
    if is_group:
        ...
        for p in process_doc.process_details:
            idx += 1
            m = get_calculated_bom(ipd, items, wo_doc.lot, process_name=p.process_name,
                                   doctype="Work Order", deliverable=deliverable)
            b.update(m)
            if idx == 0:
                first_process = p.process_name
            else:
        ...
        deliverables, y, z = calc_deliverable_and_receivable(
            ipd_doc, first_process, item_list, item_name, dept_attribute, ipd, wo_doc.lot, uom, bom, lot_doc, stiching_in_stage, pack_out_stage, pack_out_uom)
        x, receivables, total_qty = calc_deliverable_and_receivable(ipd_doc, final_process, item_list2, item_name, dept_attribute, ipd, wo_doc.lot, uom, {
        }, lot_doc, stiching_in_stage, pack_out_stage, pack_out_uom)
        main_prs = [ipd_doc.packing_process,
                    ipd_doc.stiching_process, ipd_doc.cutting_process]
        if first_process not in main_prs and final_process not in main_prs:
        ...
        bom = get_calculated_bom(ipd, items, wo_doc.lot, process_name=wo_doc.process_name,
                                 doctype="Work Order", deliverable=deliverable)
        bom = get_bom_structure(bom, row_index, table_index)
        deliverables, receivables, total_qty = calc_deliverable_and_receivable(
            ipd_doc, wo_doc.process_name, item_list, item_name, dept_attribute, ipd, wo_doc.lot, uom, bom, lot_doc, stiching_in_stage, pack_out_stage, pack_out_uom)
    ...
    if wo_doc.process_name in processes or wo_doc.process_name in dc_processes:
        ...
        complete, incomplete = get_complete_incomplete_structure(
...
def calc_deliverable_and_receivable(ipd_doc, process, item_list, item_name, dept_attribute, ipd, lot, uom, bom, lot_doc, stiching_in_stage, pack_out_stage, pack_out_uom):
    ...
    if ipd_doc.stiching_process == process:
        receivables, total_qty, table_index, row_index = get_receivables(
            item_list, lot, uom)
        stiching_attributes = get_attributes(
            item_list, item_name, stiching_in_stage, dept_attribute, ipd)
        stiching_attributes.update(bom)
        deliverables, table_index, row_index = get_deliverables(
            stiching_attributes, lot)
        accessory = get_accessories(item_list, ipd_doc)
        accessory = get_converted_accessory(
            ipd_doc, accessory, lot, table_index, row_index)
        ...
        cutting_attributes = get_attributes(
            item_list, item_name, cutting_out_stage, dept_attribute, ipd)
        accessory = get_accessories(item_list, ipd_doc)
        receivables, total_qty, table_index, row_index = get_receivables(
            cutting_attributes, lot, uom)
        accessory = get_converted_accessory(
            ipd_doc, accessory, lot, table_index, row_index)
            ...
            if item.process_name == process:
                if ipd_doc.stiching_in_stage == item.stage:
                    attributes = get_attributes(
                        item_list, item_name, item.stage, dept_attribute, ipd, process)
                    x = attributes.copy()
                    x.update(bom)
                    deliverables, table_index, row_index = get_deliverables(
                        x, lot)
                    ...
                    attributes = get_attributes(
                        item_list, item_name, item.stage, dept_attribute, pack_ipd=ipd)
                    receivables, total_qty, table_index, row_index = get_receivables(
                        attributes, lot, uom)
...
def get_converted_accessory(ipd_doc, accessory, lot, table_index, row_index):
    ...
    for acc, qty in accessory.items():
        ...
        variant_name = get_or_create_variant(
            cloth_name, {ipd_doc.packing_attribute: colour, "Dia": dia})
    ...
    for item_name, variants in items.items():
        for variant in variants:
            ...
            if conversion_details:
                uom_factor = get_uom_conversion_factor(
                    conversion_details, uom, out_uom)
...
def get_attributes(items, itemname, stage, dependent_attribute, ipd=None, process=None, pack_ipd=None):
    ...
    if pack_ipd:
        ipd_pack_doc = frappe.get_cached_doc(
            "Item Production Detail", pack_ipd)
    ...
    for item_name, variants in items.items():
        ...
        for variant in variants:
            ...
            if not ipd_doc:
                ...
                panel_colour_combination = get_panel_colour_combination(
                    ipd_doc)
                    ...
                    for item in ipd_doc.stiching_item_details:
                        ...
                        if process:
                            emb = update_if_string_instance(
                                ipd_doc.emblishment_details_json)
                            if emb.get(process):
...
def update_stock(work_order, close_reason=None, close_other_reason=None, close_remarks=None):
    ...
    if dc_list:
        frappe.throw(
            _("Delivery Challan {0} is not completed. Please complete the Delivery Challan before Close Work Order.").format(dc_list[0]))
    grn_list = frappe.get_all("Goods Received Note", filters={
        "against": "Work Order",
    ...
    if grn_list:
        frappe.throw(
            _("GRN {0} is not completed. Please complete the GRN before Close Work Order.").format(grn_list[0]))
    ...
    for data in doc.deliverables:
        if (data.qty - data.pending_quantity - data.stock_update) > 0 and res.get(data.item_variant):
            ...
            balance = get_stock_balance(
                data.item_variant, doc.supplier, received_type, lot=doc.lot)
...
def calculate_completed_pieces(doc_name):
    ...
    ipd_doc = frappe.get_doc("Item Production Detail",
                             wo_doc.production_detail)
    is_group = frappe.get_value("Process", wo_doc.process_name, "is_group")
    final_process = None
    first_process = None
    ...
    prs_list = [ipd_doc.cutting_process,
                ipd_doc.stiching_process, ipd_doc.packing_process]
...
def fetch_summary_details(doc_name, production_detail):
    ...
    deliverables = fetch_item_details(
        wo_doc.deliverables, wo_doc.production_detail)
...
def get_accessories(item_list, ipd_doc):
    ...
    for item in item_list:
        for variant in item_list[item]:
            ...
            accessory_detail = calculate_accessory(
                ipd_doc, cloth_combination, stitching_combination, attrs, variant['qty'])
...
def get_grn_rework_items(doc_name):
    ...
    for grn in grn_list:
        ...
        for item in doc.items:
            ...
            if (grn_type == "Mistake" or grn_type == "Rejected") and (item.quantity - item.rework_quantity > 0):
                ...
                items[received_type][(
                    item.get('item_variant'), set_key)]['qty'] += item.quantity
                items[received_type][(
                    item.get('item_variant'), set_key)]['grn_detail_no'].append(item.name)
    ...
    for received_type, items in deliverables.items():
        received_items = fetch_rework_popup_items(
            items, wo_doc.production_detail)
...
def create_rework(doc_name, items, supplier, supplier_address, rework_type, supplier_type):
            ...
            for item in group['items']:
                ...
                if (item.get('primary_attribute')):
                    for attr, values in item['values'].items():
                        ...
                        item1 = get_data(item, values.get("table_index"), values.get(
                            "row_index"), process_name, quantity, cost)
                        item1['item_variant'] = variant
                        item1['set_combination'] = values.get(
                            "set_combination", {})
                        item1['secondary_qty'] = values.get(
                            'secondary_qty', None)
                        item1['secondary_uom'] = values.get(
                            'secondary_uom', None)
    ...
    for item in deliverables:
        ...
        receivables[(item['item_variant'], tup)
                    ]["pending_quantity"] += item['qty']
    receivable_list = []
...
def create_finishing_detail(work_order, from_finishing=False):
    ...
    for rework in rework_list:
        ...
        for row in rework_doc.grn_rework_item_details:
            set_comb = update_if_string_instance(row.set_combination)
            key = (row.item_variant, tuple(
                sorted(update_if_string_instance(set_comb).items())))
    ...
    ipd = frappe.get_value("Lot", wo_doc.lot, "production_detail")
    pcs_per_box = frappe.get_value(
        "Item Production Detail", ipd, "packing_combo")
