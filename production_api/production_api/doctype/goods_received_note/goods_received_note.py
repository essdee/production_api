...
class GoodsReceivedNote(Document):
    ...
    def onload(self):
        if self.against == "Purchase Order":
            item_details = fetch_grn_purchase_item_details(
                self.get('items'), docstatus=self.docstatus)
            ...
            if self.includes_packing:
                ...
                ipd = frappe.get_cached_value(
                    "Work Order", self.against_id, "production_detail")
                item_details = fetch_grn_item_details(
                    items, ipd, self.lot, docstatus=self.docstatus)
                self.set_onload('item_details', item_details)
    ...
    def before_submit(self):
        ...
        if self.against == "Work Order" and self.additional_grn or self.allow_non_bundle:
            role = frappe.db.get_single_value(
                "MRP Settings", "additional_grn_submit_role")
            if not role:
                frappe.throw(
                    "Set the role for additional GRN submit in MRP Settings", title='GRN')
            user_roles = frappe.get_roles(frappe.session.user)
            if role not in user_roles:
                frappe.throw(
                    f"Only {role} can submit this document.", title='GRN')

        self.letter_head = frappe.db.get_single_value(
            "MRP Settings", "dc_grn_letter_head")
        against_docstatus = frappe.get_value(
            self.against, self.against_id, 'docstatus')
        ...
        if self.against == 'Purchase Order':
            ...
            self.freight_charge_per_product = round(
                self.freight_charges/total_qty, 2)
            self.calculate_amount()
        else:
            ...
            if self.is_return:
                ...
                lot = frappe.get_cached_value(
                    self.against, self.against_id, "lot")
                ...
                for item in self.items:
                    ...
                    for wo_item in wo_doc.deliverables:
                        ...
                        set2 = update_if_string_instance(
                            wo_item.set_combination)
                ...
                for item in self.items:
                    ...
                    reduce_stock_list.append(self.get_return_deliverables(
                        item, lot, {}, -1, default_received_type, self.supplier))
                    add_stock_list.append(self.get_return_deliverables(
                        item, lot, {}, 1, received_type, self.delivery_location))
                if self.includes_packing:
                    pack_attr, dept_attr, stich_stage = frappe.get_value("Item Production Detail", wo_doc.production_detail, [
                                                                         "packing_attribute", "dependent_attribute", "stiching_out_stage"])
                    from production_api.production_api.doctype.cut_bundle_movement_ledger.cut_bundle_movement_ledger import (
                    ...
                    for reduce_item in reduce_stock_list:
                        if check_dependent_stage_variant(reduce_item['item'], dept_attr, stich_stage):
                            variant = get_lpiece_variant(
                                pack_attr, dept_attr, reduce_item['item'])
    ...
    def update_pack_variant_items(self):
        ...
        pack_out_stage, ipd, itemname = frappe.get_value(
    ...
    def on_submit(self):
        ...
        if self.against == 'Purchase Order':
            ...
            logger.debug(
                f"{self.name} WO Receivables Updated {datetime.now()}")
            ...
            if self.is_rework:
                ...
                logger.debug(
                    f"{self.name} Deliverables Reduced from Supplier {datetime.now()}")
            ...
            if not self.additional_grn and self.against == "Work Order":
                ...
                if not self.allow_non_bundle:
                    if self.is_return and not self.cut_panel_movement:
                        ...
                        if check:
                            update_collapsed_bundle(
                                self.doctype, self.name, "on_submit")
                        ...
                        bundles, collapsed_details = get_cut_bundle_entry(
                            cpm_doc, self, from_warehouse, -1)
                        self.check_bundle_qty(bundles)
                        make_cut_bundle_ledger(bundles, collapsed_details)
                        bundles, collapsed_details = get_cut_bundle_entry(
                            cpm_doc, self, to_warehouse, 1)
                        make_cut_bundle_ledger(bundles, collapsed_details)
                    else:
                        ipd = frappe.get_value(
                            "Lot", self.lot, "production_detail")
                        stich_process = frappe.get_value(
                            "Item Production Detail", ipd, "stiching_process")
                        is_group = frappe.get_value(
                            "Process", self.process_name, "is_group")
                        check = False
                        if is_group:
                    ...
                    update_collapsed_bundle(
                        self.doctype, self.name, "on_submit", non_stich_process=True)
    ...
    def check_bundle_qty(self, bundles):
        ...
        for variant in bundle_variant_d:
            if variant not in challan_variant_d:
                frappe.throw(
                    "Unwanted bundles are selected in Cut Panel Movement")
    ...
    def generate_rework_docs(self):
        ...
        frappe.enqueue(generate_rework, "short",
    ...
    def get_to_warehouse(self):
        ...
        if self.is_internal_unit and self.supplier_address != self.delivery_address:
            to_warehouse = frappe.get_single(
                "Stock Settings").transit_warehouse

        return to_warehouse

    def piece_calculation(self):
        # calculate_pieces(self.name)
        frappe.enqueue(calculate_pieces, "short",
                       doc_name=self.name, enqueue_after_commit=True)
    ...
    def reduce_rework_stock(self):
        ...
        for item in self.items:
            ...
            for wo_item in wo_doc.deliverables:
                ...
                if item.item_variant == wo_item.item_variant and set1 == set2 and valid_qty > 0:
                    variant_received_types.setdefault(
                    ...
                    variant_received_types[(
                        item.item_variant, item.name)][wo_item.item_type]['qty'] += valid_qty
        ...
        for (variant, detail_no) in variant_received_types:
            for received_type in variant_received_types[(variant, detail_no)]:
                sl_entries.append(self.get_rework_deliverables(
                    variant, received_type, detail_no, variant_received_types, -1))

        from production_api.mrp_stock.stock_ledger import make_sl_entries
        make_sl_entries(sl_entries)

    def get_rework_deliverables(self, variant, received_type, detail_no, variant_received_types, multiplier):
        uom = variant_received_types[(
            variant, detail_no)][received_type]['uom']
    ...
    def split_items(self):
        ...
        for item in self.items:
            ...
            secondary_qty_json = update_if_string_instance(
                item.secondary_qty_json)
            ...
            if not received_types:
                ...
                for type1, qty1 in received_types.items():
                    x['quantity'] = qty1
                    x['stock_qty'] = flt(
                        flt(qty1) * flt(item.conversion_factor))
    ...
    def reduce_uncalculated_stock(self, res):
        ...
        sl_entries = []
        item_name, ipd = frappe.get_value(
            "Lot", lot, ["item", "production_detail"])
    ...
    def on_cancel(self):
        ...
        if self.against == 'Purchase Order':
            logger.debug(
                f"{self.name} On Cancel Purchase Order {datetime.now()}")
            if self.purchase_invoice_name:
                frappe.throw(
                    f'Please remove this GRN from Purchase Invoice {self.purchase_invoice_name} before cancelling. Please Contact Purchase Department.')
            ...
            if cancel_before_days and not settings.allow_grn_cancellation:
                if date_diff(frappe.utils.nowdate(), self.creation) > cancel_before_days:
                    frappe.throw(
                        f'GRN cannot be cancelled after {cancel_before_days} days of creation.', title='GRN')
                status = frappe.get_value(
                    'Purchase Order', self.against_id, 'open_status')
                if status != 'Open':
            ...
            if self.is_return:
                ...
                if self.includes_packing:
                    ...
                    for add_item in add_stock_list:
                        if check_dependent_stage_variant(add_item['item'], dept_attr, stich_stage):
                            variant = get_lpiece_variant(
                                pack_attr, dept_attr, add_item['item'])
                ...
                logger.debug(
                    f"{self.name} On Cancel {self.against} {datetime.now()}")
                wo_doc = frappe.get_doc(self.against, self.against_id)
                ...
                wo_doc.save(ignore_permissions=True)
                logger.debug(
                    f"{self.name} WO Receivable Updated {datetime.now()}")
                from production_api.production_api.doctype.delivery_challan.delivery_challan import get_variant_stock_details
                res = get_variant_stock_details()
            ...
            if not self.additional_grn and self.against == "Work Order":
                ...
                if not self.allow_non_bundle:
                    if self.is_return and not self.cut_panel_movement:
                        ...
                        if check:
                            update_collapsed_bundle(
                                self.doctype, self.name, "on_cancel")
                        ...
                        bundles, collapsed_bundles = get_cut_bundle_entry(
                            cpm_doc, self, to_warehouse, -1, cancelled=1)
                        cancel_cut_bundle_ledger(bundles)
                        bundles, collapsed_bundles = get_cut_bundle_entry(
                            cpm_doc, self, from_warehouse, 1, cancelled=1)
                        cancel_cut_bundle_ledger(bundles)
                    else:
                        ipd = frappe.get_value(
                            "Lot", self.lot, "production_detail")
                    ...
                    update_collapsed_bundle(
                        self.doctype, self.name, "on_cancel", non_stich_process=True)
                ...
                if i.name == item.ref_docname:
                    ...
                    if self.docstatus == 1:
                        validate_quantity_tolerance(
                            i.item_variant, i.qty, i.pending_qty, quantity)
    ...
    def update_stock_ledger(self):
        ...
        for item in self.items:
            ...
            sl_entries.append(self.get_sl_entries(
                item, self.delivery_location, {}, 1, received_type))
    ...
    def before_validate(self):
        ...
        if self.against == 'Purchase Order':
            ...
            lot, process, internal, includes_packing = frappe.get_cached_value(
                self.against, self.against_id, ["lot", "process_name", "is_internal_unit", "includes_packing"])
            if includes_packing and self.get('item_details'):
                ...
                docstatus = frappe.get_value(
                    "Work Order", self.against_id, "docstatus")
                if docstatus != 1:
                ...
                is_manual_entry = frappe.get_value(
                    "Process", process, "is_manual_entry_in_grn")
                if self.is_rework:
                ...
                if (self.get('item_details')) and check:
                    items, total_rate, total_qty = save_grn_item_details(
                        self.item_details, self.process_name)
                    self.set('items', items)
                    if len(self.items) > 0:
                ...
                if self.get('consumed_item_details'):
                    items = save_grn_consumed_item_details(
                        self.consumed_item_details)
                    self.set("grn_deliverables", items)
    ...
    def validate(self):
        if self.against == 'Purchase Order':
            ...
            supplier = frappe.get_value(
                'Purchase Order', self.against_id, 'supplier')
            if supplier != self.supplier:
                frappe.throw('Supplier cannot be changed.', title='GRN')

            po_docstatus = frappe.get_value(
                'Purchase Order', self.against_id, 'docstatus')
            if po_docstatus != 1:
    ...
    def validate_data(self):
        for row in self.items:
            item_details = get_uom_details(
                row.item_variant, row.uom, row.quantity)
            row.set("stock_uom", item_details.get("stock_uom"))
            row.set("conversion_factor", item_details.get("conversion_factor"))
            row.stock_qty = flt(
                flt(row.quantity) *
                flt(row.conversion_factor), self.precision("stock_qty", row)
            )
            row.stock_uom_rate = flt(
                flt(row.rate) /
                flt(row.conversion_factor), self.precision(
                    "stock_uom_rate", row)
            )
            row.amount = flt(flt(row.rate) * flt(row.quantity),
                             self.precision("amount", row))
...
def check_cut_stage_items(items, lot):
    ...
    stich_stage = frappe.get_value(
        "Item Production Detail", ipd, "stiching_in_stage")
    for item in items:
...
def fetch_consumed_item_details(items):
    ...
    for key, variants in groupby(items, lambda i: i['row_index']):
        ...
        index = get_item_group_index(
            item_details, current_item_attribute_details)
        if index == -1:
            item_details.append({
...
def save_grn_item_details(item_details, process_name):
    ...
    allowance = frappe.db.get_value(
        "Process", process_name, "additional_allowance")
    items = []
    row_index = 0
    table_index = 1
    total_rate = 0
            ...
            if (item.get('primary_attribute')):
                for attr, values in item['values'].items():
                    if values.get('ref_docname'):
                        ...
                        if received > 0:
                            total_quantity, pending_qty = frappe.get_value(values.get(
                                'ref_doctype'), values.get('ref_docname'), ["qty", "pending_quantity"])
                            ...
                            if max_qty - before_received < received:
                                frappe.throw(
                                    f"Received more than the allowed quantity for {bold(variant_name)}")

                        item1['item_variant'] = variant_name
                        item1['lot'] = item.get('lot')
                        item1['quantity'] = received
                        item1['received_types'] = values.get('types')
                        item1['secondary_qty_json'] = values.get(
                            'secondary_qty_json')
                        item1['uom'] = item.get('default_uom')
                        item1['secondary_qty'] = values.get(
                            'secondary_received')
                        ...
                        item1['ref_doctype'] = values.get('ref_doctype')
                        item1['ref_docname'] = values.get('ref_docname')
                        item1['set_combination'] = values.get(
                            'set_combination', {})
                        items.append(item1)
                        if received:
                            total_qty = total_qty + received
                            total_rate = total_rate + \
                                (received * values.get('rate'))
            else:
                if item['values'].get('default') and item['values']['default'].get('ref_docname'):
                    ...
                    total_quantity, pending_qty = frappe.get_value(
                        doctype, docname, ["qty", "pending_quantity"])
                    x = total_quantity / 100
                    x = x * allowance
                    max_qty = total_quantity + x
                    before_received = total_quantity - pending_qty
                    ...
                    item1['lot'] = item.get('lot')
                    item1['quantity'] = received
                    item1['received_types'] = item['values']['default'].get(
                        'types')
                    item1['secondary_qty_json'] = item['values']['default'].get(
                        'secondary_qty_json')
                    item1['uom'] = item.get('default_uom')
                    item1['secondary_qty'] = item['values']['default'].get(
                        'secondary_received')
                    ...
                    item1['set_combination'] = item['values']['default'].get(
                        'set_combination', {})
                    items.append(item1)
                    if received:
                        total_qty = total_qty + received
                        total_rate = total_rate + \
                            (received * item['values']['default'].get('rate'))
            row_index += 1
...
def validate_quantity_tolerance(item_variant, total_qty, pending_qty, received_qty):
    ...
    tolerance_percentage = frappe.get_value(
        "Item", item, "over_delivery_receipt_allowance") or 0
    tolerance = total_qty * (tolerance_percentage / 100)
    # Check if the tolerance is exceeded
    if (received_qty - pending_qty) > tolerance:
        frappe.throw(_("Quantity tolerance exceeded for item {0}. Received Quantity must not exceed {1}").format(
            item, pending_qty + tolerance))
    return True
        ...
        if item['primary_attribute']:
            ...
            for variant in variants:
                ...
                set_combination = update_if_string_instance(
                    variant.get('set_combination', {}))
                if set_combination:
                    if set_combination.get("major_part"):
                        item['item_keys']['major_part'] = set_combination.get(
                            "major_part")

                    if set_combination.get("major_colour"):
                        item['item_keys']['major_colour'] = set_combination.get(
                            "major_colour")
                ...
                for attr in current_variant.attributes:
                    if attr.attribute == item.get('primary_attribute'):
                        ...
                        qty = frappe.get_value(
                            variant['ref_doctype'], variant['ref_docname'], "pending_quantity")
                        if docstatus == 0:
                            item['values'][attr.attribute_value]['qty'] = round(
                                qty - variant['quantity'], 3)
                        else:
                            item['values'][attr.attribute_value]['qty'] = qty
                        item['values'][attr.attribute_value]['received'] = round(
                            variant['quantity'], 3)
            ...
            qty = frappe.get_value(
                variants[0]['ref_doctype'], variants[0]['ref_docname'], "pending_quantity")
            if docstatus == 0:
                item['values']['default']['qty'] = round(
                    qty - variants[0]['quantity'], 3)
            else:
                item['values']["default"]['qty'] = qty
            item['values']['default']['received'] = round(
                variants[0]['quantity'], 3)
    ...
    for item in item_details:
        ...
        for row_item in item['items']:
            ...
            for key in row_item['values']:
                types_json = update_if_string_instance(
                    row_item['values'][key]["types"])
...
def calculate_deliverables(grn_doc):
    ...
    ipd_doc = frappe.get_cached_doc(
        "Item Production Detail", wo_doc.production_detail)
    process = wo_doc.process_name
    final_value = {}
    if process == ipd_doc.stiching_process:
        final_value = get_stiching_process_deliverables(
            grn_doc, wo_doc, ipd_doc)
...
def get_cutting_process_deliverables(grn_doc, ipd_doc):
    ...
    accessory_json = update_if_string_instance(
        ipd_doc.accessory_clothtype_json)
    ...
    for item in grn_doc.items:
        ...
        attributes = get_receivable_item_attribute_details(
            variant_doc, item_attribute_details, ipd_doc.stiching_in_stage)
        ...
        cut_attr_key = get_key(
            attributes, cloth_combination["cutting_attributes"])
        dia, cloth_weight = cloth_combination["cutting_combination"].get(
            cut_attr_key)
        cloth_attr_key = get_key(
            attributes, cloth_combination["cloth_attributes"])
        ...
        for accessory_name, accessory_cloth in accessory_json.items():
            ...
            acc_attr_key = get_key(
                attributes, cloth_combination["accessory_attributes"])
            if cloth_combination["accessory_combination"].get(acc_attr_key):
                ...
                accessory_colour, cloth = get_accessory_colour(
                    ipd_doc, attributes, accessory_name)
...
def get_stiching_process_deliverables(grn_doc, wo_doc, ipd_doc):
    ...
    if not items:
        return
    variant_doc = frappe.get_cached_doc(
        "Item Variant", items[0]['item_variant'])
    uom = lot_doc.packing_uom
    item_list, row_index, table_index = get_item_structure(
        items, variant_doc.item, process, uom)
    bom = get_calculated_bom(lot_doc.production_detail, items,
                             lot_doc.name, process_name=process, doctype="Work Order")
    bom = get_bom_structure(bom, row_index, table_index)
    attributes = get_attributes(item_list, variant_doc.item,
                                ipd_doc.stiching_in_stage, ipd_doc.dependent_attribute, ipd)
...
def get_packing_process_deliverables(grn_doc):
    default_received_type = frappe.get_single(
        "Stock Settings").default_received_type
    ...
    if prs_doc.is_group:
        for process in prs_doc.process_details:
            for bom_item in ipd_doc.item_bom:
                if bom_item.process_name == process.process_name and not bom_item.based_on_attribute_mapping:
                    quantity, uom = get_bom(
                        bom_item, total_piece_qty, lot_item_detail, lot_doc, ipd_doc, packing_combo)
        ...
        for bom in ipd_doc.item_bom:
            if bom.process_name == grn_doc.process_name and not bom.based_on_attribute_mapping:
                quantity, uom = get_bom(
                    bom, total_piece_qty, lot_item_detail, lot_doc, ipd_doc, packing_combo)
    ...
    allow_excess = frappe.db.get_single_value(
        "MRP Settings", "allow_excess_grn")
    if allow_excess:
        for d in deliverables:
            if d['quantity'] > d['stock']:
                item = frappe.get_value(
                    "Item Variant", d['item_variant'], "item")
                if item == item_name:
                    ...
                    excess_items[d['item_variant']
                                 ]['qty'] += d['quantity'] - d['stock']
...
def get_bom(bom_item, total_piece_qty, lot_item_detail, lot_doc, ipd_doc, packing_combo):
    ...
    if bom_item.dependent_attribute_value and bom_item.dependent_attribute_value != lot_doc.pack_in_stage:
        ...
        qty_of_product = get_uom_conversion_factor(
            lot_item_detail.uom_conversion_details, dependent_attr_uom, lot_doc.packing_uom)
...
def get_attributes(items, itemname, stage, dependent_attribute, ipd):
    ...
    for item_name, variants in items.items():
        ...
        for variant, details in variants.items():
            ...
            attributes = get_receivable_item_attribute_details(
                current_variant, item_attribute_details, stage)
...
def get_panel_colour(ipd_doc):
    ...
    if ipd_doc.is_set_item:
        ...
        for row in ipd_doc.stiching_item_combination_details:
            ...
            d[part][row.major_attribute_value].setdefault(
                row.set_item_attribute_value, row.attribute_value)
    else:
        for row in ipd_doc.stiching_item_combination_details:
            d.setdefault(row.major_attribute_value, {})
            d[row.major_attribute_value].setdefault(
                row.set_item_attribute_value, row.attribute_value)
    return d
...
def get_grn_structure(doc_name):
    ...
    ipd = frappe.get_cached_value(
        "Work Order", doc.against_id, "production_detail")
    items = fetch_onload_data(doc.items)
    item_details = fetch_grn_item_details(
        items, ipd, doc.lot, docstatus=doc.docstatus)
    return item_details
...
def update_calculated_receivables(doc_name, receivables, received_type):
    ...
    for received_item in receivables:
        for item in grn_doc.items:
            set1 = update_if_string_instance(
                received_item.get('set_combination', {}))
...
def get_receivables(items, doc_name, wo_name, receivable):
    ...
    logger = get_module_logger("goods_received_note")
    logger.debug(
        f"{doc_name} Deliverables Calculation Started {datetime.now()}")
    items = get_deliverable_receivable(items, wo_name, receivable=receivable)
    logger.debug(
        f"{doc_name} Deliverables Calculation Completed {datetime.now()}")
    return items
...
def calculate_pieces(doc_name, complete_json=None, incomplete_json=None, from_pi=False):
    ...
    if doc_status == 2:
        ...
        return
    ipd, item_name = frappe.get_cached_value(
        "Lot", grn_doc.lot, ["production_detail", "item"])
    ipd_doc = frappe.get_cached_doc("Item Production Detail", ipd)
    received_types = {}
    total_received = 0
    ...
    if process_name == ipd_doc.cutting_process:
        ...
        incomplete_items, completed_items, received_types, total_received, qty_list = calculate_cutting_piece(
            grn_doc, received_types, panel_list, complete_json, incomplete_json, from_pi)
    elif check_stich_prs:
        final_calculation, received_types, total_received = calculate_piece_stage(
            grn_doc, received_types, doc_status, total_received, final_calculation)

    elif grn_doc.includes_packing:
        final_calculation, received_types, total_received = calculate_pack_stage(
            ipd_doc, grn_doc, received_types, doc_status, total_received, final_calculation, item_name)
    else:
        emb = update_if_string_instance(
            ipd_doc.get("emblishment_details_json"))
        stage = None
    ...
    for item in grn_doc.items:
        ...
        for attr in attrs:
            ...
            if doc_status == 2:
                qty = qty * -1
            variant_doc = frappe.get_cached_doc(
                "Item Variant", item.item_variant)
...
def calculate_cutting_piece(grn_doc, received_types, panel_list, complete_json, incomplete_json, from_pi):
    ...
    production_detail, incomplete_items_json, completed_items_json = frappe.get_value(
        "Work Order", grn_doc.against_id, ['production_detail', "incompleted_items_json", "completed_items_json"])
    ipd_doc = frappe.get_cached_doc(
    ...
    if from_pi:
        incomplete_items = update_if_string_instance(
            frappe.json.dumps(incomplete_json))
        completed_items = update_if_string_instance(
            frappe.json.dumps(complete_json))
    ...
    for item in grn_doc.items:
        ...
        for i in incomplete_items['items']:
            ...
            if con1 and con2:
                ...
                y = i.copy()
                if not x:
                    i['values'][attrs[ipd_doc.primary_item_attribute]][attrs[ipd_doc.stiching_attribute]] = {
                        item.received_type: item.quantity}
                else:
                    m = x.copy()
                    if m.get(item.received_type):
                        y['values'][attrs[ipd_doc.primary_item_attribute]
                                    ][attrs[ipd_doc.stiching_attribute]][item.received_type] += item.quantity
                    else:
                        y['values'][attrs[ipd_doc.primary_item_attribute]
                                    ][attrs[ipd_doc.stiching_attribute]][item.received_type] = item.quantity
                i = y
...
def generate_rework(doc_name):
    ...
    types = [single_doc.default_received_type,
             single_doc.default_rejected_type]
    items = update_if_string_instance(self.items_json)
    if self.is_return:
    ...
    for key, variants in groupby(items, lambda i: i['row_index']):
        ...
        for item in variants:
            if self.is_return:
                ...
                received_types = update_if_string_instance(
                    item['received_types'])
...
def update_finishing_item_doc(doc_name, finishing_doc_name, update_finishing: bool):
    default_type = frappe.db.get_single_value(
        "Stock Settings", "default_received_type")
    default_rejected_type = frappe.db.get_single_value(
        "Stock Settings", "default_rejected_type")
    ...
    if update_finishing and not self.is_return:
        ...
        finishing_rework_items_list = get_finishing_rework_list(
            finishing_rework_items)
        if self.is_pack:
            pack_return_list = update_if_string_instance(
                finishing_doc.pack_return_list)
            if docstatus == 2:
                del pack_return_list[doc_name]
            else:
                pack_return_list[doc_name] = frappe.utils.now_datetime().strftime(
                    "%d-%m-%Y %H:%M:%S")
            finishing_doc.pack_return_list = frappe.json.dumps(
                pack_return_list)
        else:
            if self.from_finishing:
                return_grn_list = update_if_string_instance(
                    finishing_doc.return_grn_list)
                if docstatus == 2:
                    del return_grn_list[doc_name]
                else:
                    return_grn_list[doc_name] = frappe.utils.now_datetime().strftime(
                        "%d-%m-%Y %H:%M:%S")
                finishing_doc.return_grn_list = frappe.json.dumps(
                    return_grn_list)
...
def fetch_onload_data(items):
    ...
    for item in items:
        ...
        primary_attr = frappe.get_cached_value(
            "Item", doc.item, "primary_attribute")
    ...
    for key in d:
        item_data = d[key]
        item_data['received_types'] = frappe.json.dumps(
            item_data['received_types'])
        item_data['secondary_qty_json'] = frappe.json.dumps(
            item_data['secondary_qty_json'])
