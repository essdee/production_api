...
frappe.ui.form.on("Vendor Bill Tracking", {
  async refresh(frm) {
	...
	if (frm.doc.docstatus == 0) {
	  frm.delivery_person_suggestion =
		new frappe.production.ui.SuggestedVendorBillDeliveryPerson(
	...
	if (!frm.is_new() && frm.doc.docstatus == 1) {
	  ...
	  if (show_receive && frm.doc.form_status != "Closed") {
		frm.add_custom_button(__("Bill Recieved"), () => {
		  frappe.call({
			method:
			  "production_api.production_api.doctype.vendor_bill_tracking.vendor_bill_tracking.make_bill_recieved_acknowledgement",
			args: {
			  doc_name: frm.doc.name,
			},
			callback: (r) => {
...
async function show_bill_received(frm) {
  ...
  const resp = await new Promise((resolve, reject) => {
	frappe.call({
	  method:
		"production_api.production_api.doctype.vendor_bill_tracking.vendor_bill_tracking.check_for_can_show_receive_btn",
