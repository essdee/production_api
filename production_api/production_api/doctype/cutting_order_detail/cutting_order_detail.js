...
frappe.ui.form.on("Cutting Order Detail", {
	setup(frm) {
	...
	refresh(frm) {
		...
		// Cloth Detail Vue component
		$(frm.fields_dict['cloth_detail'].wrapper).html("");
		frm.cloth_detail = new frappe.production.ui.ClothDetail(frm.fields_dict['cloth_detail'].wrapper);
		if (frm.doc.cloth_detail_json) {
			try {
				let data = JSON.parse(frm.doc.cloth_detail_json);
				frm.cloth_detail.load_data(data);
			} catch(e) {
				console.error("Failed to parse cloth_detail_json", e);
			}
		}
	...
	get_panel_combination(frm) {
		...
		frappe.call({
			method: 'production_api.production_api.doctype.cutting_order_detail.cutting_order_detail.get_co_new_combination',
			args: {
				attribute_mapping_value: frm.stiching_attribute_mapping,
				packing_attribute_details: frm.doc.attribute_values,
	...
	is_set_item(frm) {
		...
		if (frm.doc.is_set_item) {
			unhide_field('set_items_html');
			// Restore set_item_attribute from settings if missing, then init the Vue component
			if (!frm.doc.set_item_attribute) {
				frappe.db.get_single_value('IPD Settings', 'default_set_item_attribute').then(val => {
					if (val) {
						frm.set_value('set_item_attribute', val);
						frm.trigger('declarations');
						frm.trigger('make_set_combination');
					}
				});
	...
	validate: async function(frm) {
		if (frm.cloth_detail) {
			let cloth_data = frm.cloth_detail.get_data();
			frm.doc.cloth_detail_json = JSON.stringify(cloth_data);
		}
