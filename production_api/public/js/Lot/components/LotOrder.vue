  	...
  	<div ref="root">
		...
		<div>
			<div v-if='!show_parameters'>
				<button class="btn btn-success" @click="show_add_items()">Fetch Item</button>
			</div>
		</div>
		<div>
			<form @submit.prevent>
				<div>
					<div class="dependent-attr row pl-4" style="display: flex; gap: 10px"></div>
				</div>
				<div>
					<div class="primary-attr row pl-4" style="display: flex; gap: 10px"></div>
					<div class="ratio-attr row pl-4" style="display: flex; gap: 10px"></div>
					<div class="mrp-attr row pl-4" style="display: flex; gap: 10px"></div>
				</div>
				<div>
					<div class="comment-attr col-md-5"></div>
				</div>
				<div v-if="button_show">
					<button v-if="edit" class="btn btn-success" @click="add_item()">Update Item</button>
					<button v-if="!edit" class="btn btn-success" @click="add_item()">Add Item</button>
				</div>
			</form>
		</div>
  	</div>
...
function create_input_fields() {
	...
	if (list_item.value[0].primary_attribute != null && list_item.value[0].primary_attribute != "") {
		...
		list_item.value[0].primary_attribute_values.forEach((attr, ind) => {
			...
			if (edit.value) {
				qty_params[ind].set_value(list_item.value[0].items[edit_index.value]["values"][attr]['qty']);
				ratio_params[ind].set_value(list_item.value[0].items[edit_index.value]["values"][attr]['ratio'])
				mrp_params[ind].set_value(list_item.value[0].items[edit_index.value]["values"][attr]['mrp'])
			}
		});
	} 
		...
		if (edit.value) {
			qty_field.set_value(list_item.value[0].items[edit_index.value]["values"]["qty"]);
			ratio_field.set_value(list_item.value[0].items[edit_index.value]["values"]["ratio"]);
			mrp_field.set_value(list_item.value[0].items[edit_index.value]["values"]["mrp"]);
		}
	}
...
function create_dependent_attribute() {
	...
	list_item.value[0]["final_state_attr"].forEach((attr, ind) => {
		...
		if (edit.value) {
			dep_attr[ind].set_value(list_item.value[0].items[edit_index.value]["attributes"][attr]);
		}
