  ...
  <div ref="root" class="ppo-root">
    <div class="ppo-header">
      ...
      <div class="ppo-controls">
        ...
        <div class="ppo-actions">
          <button class="btn btn-primary btn-get" @click="get_report">
            Get Report
          </button>
          <button
            class="btn btn-default btn-dl"
            @click="download_excel"
            v-if="flat_orders.length > 0"
          >
            <svg
              class="download-icon"
              width="14"
    ...
    <div v-if="flat_orders.length > 0" class="ppo-body">
      ...
      <div class="ppo-toggles">
        ...
        <label class="ppo-toggle"
          ><input type="checkbox" v-model="show_fabric" /> Show Fabric</label
        >
        <label class="ppo-toggle"
          ><input type="checkbox" v-model="show_dia" /> Show Dia</label
        >
        <label class="ppo-toggle"
          ><input type="checkbox" v-model="show_gsm" /> Show GSM</label
        >
          ...
          <thead>
            ...
            >
              ...
              <template v-if="sgIdx === 0">
                ...
                <th :rowspan="size_groups.length">Quantity Transfer</th>
          ...
          <tbody>
            <template v-for="(order, idx) in flat_orders" :key="order.name">
              <tr>
                ...
                <td>
                  <a
                    :href="'/app/production-order/' + order.name"
                    class="ppo-link"
                  >
                    {{ order.name }}
                  </a>
                ...
                <td>
                  <button
                    class="btn btn-xs btn-default btn-summarize"
                    @click="show_summary(order.name)"
                  >
                    {{
                ...
                <td class="cell-group-label" :title="order.group_label">
                  {{ order.group_label }}
                </td>
                <td
                  v-for="colIdx in max_cols"
                ...
                <td class="cell-transfer">
                  <span
                    v-if="!order.transfer_movements?.length"
                    class="cell-zero"
                    >—</span
                  >
                  <div
                    v-for="movement in order.transfer_movements"
                    :key="movement.transfer_reference + movement.movement"
                  ...
                  >
                    ...
                    <span>{{
                      movement.movement === "Reduced" ? "to" : "from"
                    }}</span>
                    <a
                      :href="
                        '/app/production-order/' +
                        movement.counterpart_production_order
                      "
                      class="ppo-link transfer-link"
                    >
                      {{ movement.counterpart_production_order }}
                    </a>
                  </div>
                </td>
                <td class="cell-comments" :title="order.comments">
                  {{ order.comments }}
                </td>
              </tr>
              <template v-if="summaryState[order.name]?.expanded">
                <tr
                  v-if="summaryState[order.name]?.loading"
                  class="summary-row"
                >
                  <td :colspan="full_colspan" class="summary-status">
                    Loading...
                  </td>
                </tr>
                <template v-else>
                  <tr
                    v-if="!summaryState[order.name]?.data?.rows?.length"
                    class="summary-row"
                  >
                    <td :colspan="full_colspan" class="summary-status">
                  ...
                  <template v-else>
                    <tr
                      v-for="(row, rIdx) in summaryState[order.name].data.rows"
                      :key="'s-' + order.name + '-' + rIdx"
                      class="summary-row"
                    >
                      ...
                      <td
                        ...
                        :class="{
                          'cell-zero':
                            !row.sizes[order.group_sizes[colIdx - 1]],
                        }"
                      >
                        {{
                          (order.group_sizes[colIdx - 1] &&
                            row.sizes[order.group_sizes[colIdx - 1]]) ||
                          "—"
                        }}
                      </td>
                    ...
                    <tr class="summary-footer-row">
                      <td
                        ...
                        class="summary-label-cell summary-footer-label"
                      >
                        Dispatched Total
                      </td>
                      <td
                        v-for="colIdx in max_cols"
                        :key="'sf-' + colIdx"
                        class="summary-footer-num"
                      >
                        {{
                          summarySizeTotalByPos(
                            order.name,
                            order.group_sizes[colIdx - 1]
                          )
                        }}
                      </td>
                      <td class="summary-footer-num summary-footer-grand">
                        {{ summaryTotal(order.name) }}
                      </td>
                      <td></td>
                      <td></td>
                    </tr>
                  </template>
                  <template v-if="order.transfer_movements?.length">
                    <tr class="transfer-history-heading-row">
                      <td :colspan="full_colspan">Quantity Transfer History</td>
                    </tr>
                    <tr
                      v-for="movement in order.transfer_movements"
                      :key="
                    ...
                    >
                      <td
                        :colspan="label_colspan"
                        class="transfer-history-label"
                      >
                        {{ formatDateTime(movement.approved_on) }} &middot;
                      ...
                      <td
                        :class="[
                          'transfer-history-total',
                          movement.movement === 'Reduced'
                            ? 'movement-reduced'
                            : 'movement-added',
                        ]"
                      >
                        {{ formatSignedQuantity(movement.signed_quantity) }}
                      </td>
                      <td colspan="2" class="transfer-history-meta">
                        Approved by {{ movement.approved_by || "Unknown" }}
                        <span v-if="movement.reason">
                          &middot; {{ movement.reason }}</span
                        >
                      </td>
                    </tr>
                  </template>
    ...
    <div v-else-if="fetched" class="ppo-empty">
      ...
      <div class="ppo-empty-copy">
        The selected filters did not return any PPO records.
      </div>
    </div>
  </div>
</template>
...
const fixed_col_count = computed(
  () =>
    9 +
    (show_fabric.value ? 1 : 0) +
    (show_dia.value ? 1 : 0) +
    (show_gsm.value ? 1 : 0)
);
// group-label cell only
const label_colspan = computed(() => fixed_col_count.value + 1);
// everything: fixed cols + group-label + size cols + Total + Transfer + Comments
const full_colspan = computed(() => fixed_col_count.value + max_cols.value + 4);
...
function movementQuantity(movement, sizeName) {
  if (!sizeName) return "—";
  return formatSignedQuantity(movement.signed_sizes?.[sizeName]);
}
...
function download_excel() {
  ...
  window.open(
    `/api/method/production_api.utils.download_ppo_report?${params.toString()}`
  );
}
...
.transfer-badge.reduced {
  color: #b91c1c;
  background: #fef2f2;
}

.transfer-badge.added {
  color: #047857;
  background: #ecfdf5;
}
...
.transfer-history-heading-row td {
  padding: 7px 12px !important;
  color: #475569;
  background: #f8fafc;
  border-bottom: 1px solid var(--ppo-border);
  font-size: 10.5px !important;
  font-weight: 800;
  letter-spacing: 0.04em;
  text-align: left !important;
...
.transfer-history-row td {
  background: #fffdf7;
  border-bottom: 1px solid #fde7bd;
  font-size: 11.5px;
}

.transfer-history-row:hover td {
